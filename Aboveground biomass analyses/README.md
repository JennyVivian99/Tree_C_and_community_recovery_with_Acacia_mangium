## Folder and code content
In this folder are provided the datasets used in [publication].
Analyses on the aboveground biomass are described in the code. Specifically, are present:
- Linear Mixed Models (LMMs)
- Generalised Linear Models (GLMs)
These are calculated for the biomass of trees obtained from the equation of Chave et al. (2005,2014), with and without the consideration of height (see [publication] for details on calculations and related methodology).
In addition, LMMs and GLMs are calculated for the aboveground biomass calculated through Chave et al. (2014) but without the consideration of dead trees.

## References
- Chave, J. et al. Tree allometry and improved estimation of carbon stocks and balance in tropical forests. Oecologia 145, 87–99 (2005). 
- Chave, J., Réjou‐Méchain, M., Búrquez, A., Chidumayo, E., Colgan, M. S., Delitti, W. B., ... & Vieilledent, G. (2014). Improved allometric models to estimate the aboveground biomass of tropical trees. Global change biology, 20(10), 3177-3190. 
